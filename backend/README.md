...
## Локальный помощник (первый этап)

Помощник отвечает на вопросы об этапах и создании отчётов через локальную Ollama. Ему передаются только текущие названия, сроки и подсказки этапов, а также краткие правила интерфейса. Чат не читает записи вузов и не выполняет действия.

Установите Ollama на той же машине, где работает FastAPI, и загрузите модель:
...
После запуска Ollama и API войдите в приложение и откройте кнопку «Помощник» справа снизу. `POST /api/v1/assistant/chat` требует Bearer-токен. Пример тела: `{"message":"Как сменить этап?","history":[],"page":"interactions"}`. При недоступной Ollama чат показывает ошибку, остальные разделы продолжают работать.

Настройки: `OLLAMA_BASE_URL`, `OLLAMA_MODEL`, `ASSISTANT_ENABLED`, `ASSISTANT_TIMEOUT_SECONDS`. По умолчанию используется `http://127.0.0.1:11434` и `qwen3:4b`; порт Ollama должен оставаться доступным только локально или из внутренней сети API.
